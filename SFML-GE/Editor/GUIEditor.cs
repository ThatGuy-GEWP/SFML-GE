...
namespace SFML_GE.Editor
{
    ...
    /// <summary>
    /// A Simple Editor/Debugger.
    /// When added to a GameObject in a Scene as a Component, it will construct
    /// A GUI based editor thats shows every GameObject in a scene and its components, including itself.
    /// you can also edit and see any public values on components attached to a GameObject.
    /// Also shows all resources in a project.
    /// </summary>
    public class GUIEditor : GUIInteractiveWindow
    {
        ...
        GUIPanel HandleMember(ClassMemberInfo member, Component memberSource, Component addTo, Type? valType, object? value, string valueName)
        {
            ...
            if (valType == typeof(UDim2) && createdVal == false)
            {
                ...
                GUIPanel holder = AddToObj(NewInvisiblePanel(), "udimHolder", panel);
            ...
            if (valType == typeof(bool) && createdVal == false)
            {
                ...
                EditorUpdate += (ed) =>
                {
                    ...
                    if (curState)
                    {
                        button.backgroundColor = defaultForeground;
                    }
        ...
        void SetupGameObjectInfo(GameObject go)
        {
            ...
            foreach (Component comp in go.Components)
            {
                ...
                EditorUpdate += (ed) =>
                {
                    compEnable.backgroundColor = comp.Enabled ? defaultPrimary : defaultPressed;
                };
